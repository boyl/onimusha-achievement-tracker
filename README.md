...
版本 0.4.3。支持 Windows Steam 正式版；已验证 build 24769601、REFramework TDB 82。英文说明见 README_EN.md。
...
- **F8**：默认显示或隐藏追踪小面板。在“显示设置与帮助”点击改键，然后按想用的键；Esc 取消，可清除绑定或恢复 F8。不支持组合键。
- 首次启动默认不弹出追踪窗口；可从 REFramework 的脚本菜单点击“打开成就与收集追踪”。Reset scripts 恢复重载前的面板开关状态。
- 编辑框架菜单时不会触发小面板快捷键；开始游戏操作前请收起整个 REFramework 菜单。
...
有用户报告新周目首关扫描交互无法完成。本次尚未复现，未宣称修复；请提供具体地点、输入设备及关闭整个 REFramework 菜单后是否恢复。若 Insert 连 REFramework 菜单都无法打开，需要检查框架加载及其菜单键配置。
